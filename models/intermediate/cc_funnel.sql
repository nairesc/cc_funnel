WITH clean_cc_funnel AS (SELECT * FROM {{ ref('stg_raw__cc_funnel') }})

    ,clean_cc_funnel_priority AS (SELECT * FROM {{ ref('stg_raw__priority') }})
...
FROM clean_cc_funnel f
LEFT JOIN clean_cc_funnel_priority p ON f.company = p.company_name
